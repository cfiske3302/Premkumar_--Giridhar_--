...
class TetrisInterface(abc.ABC):
    ...
    def __init__(self, gridchoice="none", obs_type="image", mode="rgb_array"):
        ...
        self.timer2p = (
            pygame.time.Clock()
        )  # this will be used for counting down time in our game
    ...
    def reset(self):
        ...
        for i, player in enumerate(self.tetris_list):
            ...
            com_event.reset()
            self.renderer.drawByName(
                "gamescreen", pos["gamescreen"][0], pos["gamescreen"][1]
            )  # blitting the main background
...
class TetrisSingleInterface(TetrisInterface):
    ...
    def act(self, action):
        ...
        player, opponent = (
            self.tetris_list[self.now_player],
            self.tetris_list[::-1][self.now_player],
        )
        tetris = player["tetris"]
        com_event = player["com_event"]
        pos = player["pos"]
        ...
        if tetris.check_fallen():
            ...
            self.renderer.drawBack2Back(
                tetris, pos["back2back"][0], pos["back2back"][1]
            )
            ...
            if tetris.check_KO():
                ...
                self.renderer.drawByName(
                    "transparent", pos["transparent"][0], pos["transparent"][1]
                )
